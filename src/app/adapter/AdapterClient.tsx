...
/* ═══════════════════════════════════════════════════════════════
   ...
   v4.1 CHANGES:
   - propsExtra support: JSON key "propsExtra" with mirror/contactOnly/dealOnly
     arrays. Processed independently of tieneCotizador flag.
     Use case: SICO migration props, custom integration fields.
   - Fix 1: 409 on Custom Object now resolves existing schema via GET,
     so associations work on rerun (not just first deploy).
   - Fix 2: Native object types (contacts=0-1, companies=0-2, deals=0-3,
     tickets=0-5) resolved explicitly for associations. "agrupacion → deals"
     now works correctly.
   - Fix 3: All Custom Object enum values normalized via toVal() at deploy
     time. Prevents sync mismatches (e.g. "Disponible" vs "disponible").
   - Fix 4: On 409 for properties, GET existing and compare type/fieldType/
     options. Mismatches logged as errors, not silently skipped.
   - Fix 5: toVal() hardened — strips special chars ($, /, etc), trims edges.
   - Fix 6: Download permissions button disabled until JSON is loaded.
   - hasSinco now true when propsExtra is present (not only tieneCotizador).
   - handleFileUpload validates pipeline same as handleParseJson.
   ═══════════════════════════════════════════════════════════════ */
...
function buildDeployPlan(config) {
  const plan = {
    ...
    hasSinco: config.tieneCotizador === true || !!(config.propsExtra),
    scopes: [],
    propertyGroups: [],
  ...
  // ─── v4.1: propsExtra — additional props from migration/integration ───
  // Independent of tieneCotizador — always processed if present in JSON
  if (config.propsExtra) {
    if (config.propsExtra.mirror && config.propsExtra.mirror.length > 0) {
      plan.mirrorProperties.push(...config.propsExtra.mirror);
    }
    if (config.propsExtra.contactOnly && config.propsExtra.contactOnly.length > 0) {
      plan.contactOnlyProperties.push(...config.propsExtra.contactOnly);
    }
    if (config.propsExtra.dealOnly && config.propsExtra.dealOnly.length > 0) {
      plan.dealOnlyProperties.push(...config.propsExtra.dealOnly);
    }
  }
...
export default function FocuxAdapter() {
  ...
  const runDeploy = async () => {
    ...
    const isCancelled = () => cancelRef.current || signal.aborted;

    const createGroup = async (objectType, groupName, groupLabel) => {
      if (isCancelled()) return;
      const r = await hsCall(token, "POST", `/crm/v3/properties/${objectType}/groups`, { name: groupName, label: groupLabel }, signal);
    ...
    const createProp = async (objectType, prop) => {
      ...
      const r = await hsCall(token, "POST", `/crm/v3/properties/${objectType}`, body, signal);
      if (r.ok) {
        log("ok", `Propiedad '${prop.name}' creada en ${objectType}`);
      } else if (r.status === 409) {
        // v4.1 Fix 4: Validate existing property matches expected schema
        const existing = await hsCall(token, "GET", `/crm/v3/properties/${objectType}/${prop.name}`, null, signal);
        if (existing.ok) {
          const diffs = [];
          if (existing.data.type !== prop.type) diffs.push(`type: ${existing.data.type} → ${prop.type}`);
          if (existing.data.fieldType !== prop.fieldType) diffs.push(`fieldType: ${existing.data.fieldType} → ${prop.fieldType}`);
          if (prop.options && prop.options.length > 0) {
            const existingValues = new Set((existing.data.options || []).map(o => o.value));
            const missing = prop.options.filter(o => !existingValues.has(o.value));
            if (missing.length > 0) diffs.push(`opciones faltantes: ${missing.map(o => o.value).join(", ")}`);
          }
          if (diffs.length > 0) {
            log("err", `CONFLICTO '${prop.name}' en ${objectType}: ${diffs.join(" | ")}. Revisar manualmente.`);
          } else {
            log("skip", `Propiedad '${prop.name}' ya existe en ${objectType} (schema OK)`);
          }
        } else {
          log("skip", `Propiedad '${prop.name}' ya existe en ${objectType} (no se pudo verificar schema)`);
        }
      } else {
        log("err", `Error '${prop.name}' en ${objectType}: ${r.data?.message || JSON.stringify(r.data)}`);
      }
    };

    try {
      addLog("info", `═══ FOCUXAI ADAPTER v4.1 — Deploying: ${plan.clientName} ═══`);
      addLog("info", `Modo: ${plan.hasCotizador ? "Cotizador + Sinco + Custom Objects" : "HubSpot Nativo (sin Custom Objects)"}${plan.hasSinco && !plan.hasCotizador ? " + propsExtra SICO" : ""}`);
      addLog("info", `v4.1: Propiedades espejo + validación 409 + AbortController`);
      ...
      // PHASE 3: Contact-Only Properties
      if (!isCancelled() && plan.contactOnlyProperties.length > 0) {
        addLog("info", `── Fase 3: Propiedades solo Contacto (${plan.contactOnlyProperties.length}) ──`);
        for (const prop of plan.contactOnlyProperties) {
          if (isCancelled()) break;
          await createProp("contacts", prop);
        }
      } else if (plan.contactOnlyProperties.length === 0) {
        addLog("info", "── Fase 3: Propiedades solo Contacto — SKIPPED ──");
      }

      // PHASE 4: Deal-Only Properties
      if (!isCancelled() && plan.dealOnlyProperties.length > 0) {
        addLog("info", `── Fase 4: Propiedades solo Deal (${plan.dealOnlyProperties.length}) ──`);
        for (const prop of plan.dealOnlyProperties) {
          if (isCancelled()) break;
          await createProp("deals", prop);
        }
      } else if (plan.dealOnlyProperties.length === 0) {
        addLog("info", "── Fase 4: Propiedades solo Deal — SKIPPED ──");
      }

      // PHASE 5: Pipeline
      if (!isCancelled() && plan.pipeline) {
        ...
        const r = await hsCall(token, "POST", "/crm/v3/pipelines/deals", pipelineBody, signal);
      ...
      if (!isCancelled() && plan.hasCustomObjects && plan.customObjects.length > 0) {
        ...
        for (const schema of plan.customObjects) {
          ...
          const r = await hsCall(token, "POST", "/crm/v3/schemas", schemaBody, signal);
          if (r.ok) {
            ...
            // v4.1 Fix 1: Resolve existing schema so associations work on rerun
            const existing = await hsCall(token, "GET", `/crm/v3/schemas/${schema.name}`, null, signal);
            if (existing.ok) {
              createdObjects[schema.name] = existing.data;
              log("ok", `Schema '${schema.name}' resuelto (objectTypeId: ${existing.data.objectTypeId})`);
            } else {
              log("err", `No se pudo resolver schema existente '${schema.name}': ${existing.data?.message || existing.status}`);
            }
          } else {
        ...
        addLog("info", "── Fase 6: Custom Objects — SKIPPED (sin Custom Objects) ──");
      }

      // PHASE 6.5: Safety net — resolve any missing schemas before associations
      // v4.1: GET /crm/v3/schemas to catch anything not in createdObjects
      if (!isCancelled() && plan.hasCustomObjects && plan.associations.length > 0) {
        const neededNames = new Set(plan.associations.flatMap(a => [a.from, a.to]));
        const NATIVE_NAMES = new Set(["contacts", "companies", "deals", "tickets"]);
        const missingCustom = [...neededNames].filter(n => !NATIVE_NAMES.has(n) && !createdObjects[n]);

        if (missingCustom.length > 0) {
          addLog("info", `── Fase 6.5: Resolviendo ${missingCustom.length} schema(s) faltantes ──`);
          const allSchemasRes = await hsCall(token, "GET", "/crm/v3/schemas", null, signal);
          if (allSchemasRes.ok && Array.isArray(allSchemasRes.data?.results)) {
            for (const s of allSchemasRes.data.results) {
              if (missingCustom.includes(s.name) && !createdObjects[s.name]) {
                createdObjects[s.name] = s;
                log("ok", `Schema '${s.name}' resuelto via GET masivo (objectTypeId: ${s.objectTypeId})`);
              }
            }
          }
          // Check if any are still missing
          const stillMissing = missingCustom.filter(n => !createdObjects[n]);
          if (stillMissing.length > 0) {
            log("err", `Schemas no resueltos: ${stillMissing.join(", ")}. Asociaciones con estos objetos fallarán.`);
          }
        }
      }

      // PHASE 7: Associations
      // v4.1 Fix 2: Native object type map
      const NATIVE_OBJECT_TYPES = { contacts: "0-1", companies: "0-2", deals: "0-3", tickets: "0-5" };

      if (!isCancelled() && plan.associations.length > 0) {
        addLog("info", `── Fase 7: Asociaciones (${plan.associations.length}) ──`);
        for (const assoc of plan.associations) {
          if (isCancelled()) break;

          const resolveObjectTypeId = (name) => {
            if (NATIVE_OBJECT_TYPES[name]) return NATIVE_OBJECT_TYPES[name];
            if (createdObjects[name]?.objectTypeId) return createdObjects[name].objectTypeId;
            return null;
          };

          const fromId = resolveObjectTypeId(assoc.from);
          const toId = resolveObjectTypeId(assoc.to);

          if (fromId && toId) {
            const r = await hsCall(token, "POST", `/crm/v4/associations/${fromId}/${toId}/labels`, {
              label: assoc.label, name: assoc.label.toLowerCase().replace(/\s+/g, "_"),
            }, signal);
            if (r.ok) log("ok", `Asociación '${assoc.label}' creada (${fromId} → ${toId})`);
            ...
            const missing = [];
            if (!fromId) missing.push(assoc.from);
            if (!toId) missing.push(assoc.to);
            log("err", `Asociación '${assoc.label}' no se puede crear — objeto(s) no resueltos: ${missing.join(", ")}`);
          }
        }
      ...
      addLog("info", `v4.1 Resumen: ${plan.mirrorProperties.length} props espejo (×2 = ${totalMirror} ops) + ${totalContactOnly} solo contacto + ${totalDealOnly} solo deal`);
      if (stats.err > 0) {
        addLog("err", `⚠ ${stats.err} error(es) detectados. Revisar logs arriba antes de usar el portal.`);
      }
    } catch (e) {
      if (e.name === "AbortError") {
        addLog("info", "═══ DEPLOY CANCELADO — fetch abortado por el usuario ═══");
      } else {
    ...
    <div>
      ...
      <div style={{ ...ss.card, borderColor:tk.accent, borderWidth:2 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:16 }}>
          ...
          <button style={{ ...ss.btn(tk.accent, "#fff"), opacity: plan ? 1 : 0.5 }} disabled={!plan} onClick={downloadPermissions}>↓ Descargar</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={ss.card}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            {deploying && (
              <button style={ss.btn(tk.red, "#fff")} onClick={() => { cancelRef.current = true; if (abortRef.current) abortRef.current.abort(); }}>✕ Cancelar</button>
            )}
            {deployDone && (
              <>
